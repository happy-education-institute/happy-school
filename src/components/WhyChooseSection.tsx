...
const reasons = [
  { key: "r1", icon: ShieldCheck },
  { key: "r2", icon: HeartHandshake },
  { key: "r3", icon: BookOpen },
  { key: "r4", icon: Sparkles },
    ...
    <section className="py-16 md:py-24 bg-gradient-to-br from-white via-school-sage/30 to-school-peach/30 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-10">
          <div>
            <p className="text-sm font-semibold text-secondary uppercase tracking-wide">{t('whyChoose', 'subtitle')}</p>
          ...
          <div className="flex items-center gap-3 bg-white/70 backdrop-blur rounded-2xl px-4 py-3 shadow-sm border border-white/60">
            ...
            <span className="text-sm font-semibold text-foreground">{t('whyChoose', 'trusted')}</span>
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
          ...
          {reasons.map((item, idx) => (
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-foreground">{t('whyChoose', item.key + 'Title')}</h3>
                  <p className="text-sm text-muted-foreground mt-1 leading-relaxed">{t('whyChoose', item.key + 'Desc')}</p>
                </div>
              </div>
            </motion.div>
          ))}
        ...
        >
          ...
          <div className="relative flex items-center gap-3 mb-6">
            ...
            <div>
              <p className="text-xs uppercase tracking-wide text-primary-foreground/70">{t('whyChoose', 'achievements')}</p>
              <h3 className="font-heading text-2xl">{t('whyChoose', 'results')}</h3>
            </div>
          </div>
          <div className="space-y-4 relative">
            <div className="absolute left-3 top-1 bottom-1 w-[2px] bg-primary-foreground/30" />
            {achievements.map((item, idx) => (
              <div key={item.key} className="pl-8 relative">
                <div className="absolute left-0 top-2 w-3 h-3 rounded-full bg-white shadow-sm" />
                <h4 className="font-semibold text-lg leading-tight">{t('whyChoose', item.key + 'Title')}</h4>
                <p className="text-sm text-primary-foreground/80 leading-relaxed">{t('whyChoose', item.key + 'Desc')}</p>
              </div>
            ))}
          </div>
        </motion.div>
